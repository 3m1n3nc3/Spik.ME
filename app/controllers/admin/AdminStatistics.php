...
class AdminStatistics extends Controller {
    ...
    protected function payments() {
        ...
        $payments_chart = [];
        $result = $this->database->query("SELECT COUNT(*) AS `total_payments`, DATE_FORMAT(`date`, '%Y-%m-%d') AS `formatted_date`, TRUNCATE(SUM(`amount`), 2) AS `total_amount` FROM `payments` WHERE `date` BETWEEN '{$this->date->start_date_query}' AND '{$this->date->end_date_query}' GROUP BY `formatted_date`");
